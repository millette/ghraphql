...
const extractKeywords = (str, limit) => {
  // const allWords = words(str.toLowerCase().replace(/[’']/g, ' ')).filter((a) => a.length >= 2)
  const allWords = words(deburr(str.toLowerCase().replace("'", ' '))).filter(
    a => a.length >= 2
  )
  // const allWords = words(deburr(str.toLowerCase())).filter((a) => a.length >= 2)

  /*
  const detectedLanguage = (franc(str)).slice(0, 2)
  const validWords = countBy(supported[detectedLanguage]
